...
const UploadComplaints = () => {
  ...
  useEffect(() => {
    const getComplaint = async () => {
      ...
      const res = await axios.get(
        `http://localhost:5000/complaint/${params.id}`
      );
      const data = res.data;
      setNewUser({
        name: data.name,
  ...
  const onSubmit = (e) => {
    ...
    if (params.id) {
      axios
        .put(`http://localhost:5000/complaint/update/${params.id}`, formData)
        .then((res) => {
          console.log(res);
          history.goBack();
        });
      ...
      axios
        .post('http://localhost:5000/complaint/addcomplaint', formData)
      ...
      <Container className='center'>
        <Row className='content'>
          <div className='col-md-6 mb-3 mx-auto'>
            <h3>{params.id ? 'Update' : 'Add'} Complaint</h3>
            <form
              method='POST'
              onSubmit={onSubmit}
            ...
            >
              ...
              <div className='form-group row'>
                <label> Image </label>
                <br />
                {newUser.photo.length > 0
                  ? newUser.photo.split('complaintimage/')[1]
                  : ''}
                <input
                  type='file'
                  accept='.png, .jpg, .jpeg'
              ...
              <div className='form-group '>
                <input
                  type='submit'
                  value={`${params.id ? 'Update' : 'Create'} Complaint`}
